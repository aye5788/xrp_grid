...
def validate_persona_references(persona_text: str, agent_id: str) -> dict:
    ...
    error_tokens: dict = {}  # token -> reason
    warn_tokens: dict = {}   # token -> reason
    ...
    for m in _BARE_NAME_RE.finditer(body):
        ...
        if not candidates:
            error_tokens[token] = (
                f"persona references {token!r} — not in schema "
                f"(closest declared paths: {_closest_paths(token)})"
            )
            continue
...
PERSONA_DIR = Path(__file__).resolve().parent / "prompts"


def load_persona(agent_id: str) -> str:
    """Read the on-disk persona file for an agent."""
    path = PERSONA_DIR / f"{agent_id}_prompt.txt"
